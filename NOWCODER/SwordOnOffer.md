...
* 树
    ...
    * <a href="#tree3">数据流中的中位数</a>（平衡二叉树//堆
* 字符串
...
* 题解：
    ...
    * 方法二： 异或法。 任何数异或本身都得0 ，任何数异或0都等于它本身
        1. 整个数组异或，得到两个只出现一次的数组异或的值
        2. 找到该数字为1的最低位（两个数字出现不同的位）
        3. 凭借该位的值 将数组分成两组，分别异或，则得到最终的两个数字


### 机器人的运动范围
* 考点：回溯法
* 知识点：数组
* program: [MovingCount.py](MovingCount.py)
* 题解：将地图全部置1，遍历能够到达的点，将遍历的点置0并令计数+1.


### <a id="tree3">数据流中的中位数</a>
* 考点：树
* 知识点：平衡二叉树 堆
* program: [GetMedian.java](GetMedian.java)(该题python的测试有问题)
* 题解：
    1. 平衡二叉树
    2. 建立一个大顶堆一个小顶堆 小堆存大数，大堆存小数

### 平衡二叉树
* 考点： 树
* 知识点：平衡二叉树
* program: [IsBalancedTree.java](IsBalanceTree.java)
* 题解：
...
* 题解：
  ...
  * 方法二：
    *  先把所有的left[i]求出，right[i]求出。
  
### 不用加减乘除做加法
* 考点：数学
* 知识点：进制转化
* program：[Add.java](Add.java)
* 题解：
...
* 考点：队列 树
* 知识点：队列 树
* program: [PrintTreeByLine.java](PrintTreeByLine.java)
* 题解：
    * 用队列实现广度优先遍历二叉树
    * 分层：记录每层的节点数sz，循环sz次即可。

### 二叉树的下一个结点
* 考点：树
* 知识点：中序遍历二叉树
* program: [NextNode.java](NextNode.java)
* 题解：
    * 方法一：找到该树的根节点，中序遍历该树。找到节点在中序遍历中的位置，返回下一个节点。
    * 方法二：
        1.二叉树为空，则返回空；
        2.节点右孩子存在，则为右孩子这和树中序遍历的第一个节点，即：右孩子向左找到叶子节点，如果没有叶子节点则是右孩子自己。
        3.节点没有父节点（节点为根节点），则没有下一个节点。
        4.节点是左孩子，下一个节点为其父节点。
        5.对节点的父节点重复3，4，5。


### <a id="linklist2">链表中环的入口结点</a>
* 考点：链表
* 知识点：快慢指针
* program: [EntryNodeOfTheLoop.java](EntryNodeOfTheLoop.java)
* 题解：
  * 参考链接：https://www.nowcoder.com/questionTerminal/253d2c59ec3e4bc68da16833f79a38e4?f=discussion
    1. 找环中相汇点。分别用p1，p2指向链表头部，p1每次走一步，p2每次走二步，直到p1==p2找到在环中的相汇点。
    2. 找环的入口。接上步，当p1==p2时，p2所经过节点数为2x,p1所经过节点数为x,设环中有n个节点,p2比p1多走一圈有2x=n+x; n=x;可以看出p1实际走了一个环的步数，再让p2指向链表头部，p1位置不变，p1,p2每次走一步直到p1==p2; 此时p1指向环的入口。


### 字符流中第一个不重复的字符
* 考点：字符串
* 知识点：hashmap
* program: [FirstCharAppearingOnce.java](FirstCharAppearingOnce.java)
* 题解：
  * 方法一：
    * 使用hashmap保存每种字符出现的次数，使用LinkedList（链表）维护一个只出现一次的字符的队列。
    * 每加入一个字符，在hashmap中记录出现的次数，判断该字符出现了几次，如果只出现了一次则加入队列的末尾。如果出现了两次则从队列中删除。
    * 队列的对首即为第一个不重复的字符。
  * 方法二：（改进）
    * 使用hashmap保存每种字符出现的次数，创建一个插入的字符的队列。 
    * 每插入一个字符,在hashmap中记录出现的次数
    * 查询第一个不重复字符时，查询队列的首位出现的次数如果不止一次则删除，并访问下一位。

### 数组中重复的数字
* 考点：数组
* 知识点：数组
* program：[DuplicateNumbers.java](DuplicateNumbers.java)
* 题解：
  * 方法一：用一个长度同样为length的数组储存每个数字出现的次数。如果次数大于1，返回该数字。
  * 方法二：不适用额外的数组。当一个数字被访问过后，设置对应为上的数+n，再遇到相同数字是，对应位上的数字大于n，则返回这个数。

### 和为S的连续正数序列
* 考点：穷举
* 知识点：穷举 数学 双指针 滑动窗口
* program: 
  * 方法一: [FindContinuousSequenceOfSumS.java](FindContinuousSequenceOfSumS.java)
  * 方法二: [FindContinuousSequenceOfSumS2.java](FindContinuousSequenceOfSumS2.java)
* 题解：
  * 方法一：(数学)
  ...
  * 方法二: (双指针 滑动窗口)
    * 设置一个窗口，窗口的左右两边就是两个指针，我们根据窗口内值之和来确定窗口的位置和宽度，如果和小于S，右移大指针，大于S右移小指针。